...
void issue_debug_snapshot(SubMaster &sm) {
  ...
  s.scene.display_debug_alert_frame = sm.frame;
}
*/
void OnroadWindow::mousePressEvent(QMouseEvent* e) {
  ...
  //const SubMaster &sm = *(s->sm); //TODO XXXX not sure about this one
  //auto longitudinal_plan = sm["longitudinalPlan"].getLongitudinalPlan();
  ...
  if (longitudinal_plan.getSpeedLimit() > 0.0 && speed_limit_touch_rect.contains(e->x(), e->y())) {
    ...
    s->scene.last_speed_limit_sign_tap = seconds_since_boot();
    Params().putBool("LastSpeedLimitSignTap", 1);
    Params().putBool("LastSpeedLimitSignTap", 0);
    s->scene.speed_limit_control_enabled = !s->scene.speed_limit_control_enabled;
    Params().putBool("SpeedLimitControl", s->scene.speed_limit_control_enabled);
...
ButtonsWindow::ButtonsWindow(QWidget *parent) : QWidget(parent) {
  ...
  QObject::connect(dlpBtn, &QPushButton::clicked, [=]() {
    s->scene.dynamic_lane_profile = s->scene.dynamic_lane_profile + 1;
    if (s->scene.dynamic_lane_profile > 2) {
      s->scene.dynamic_lane_profile = 0;
    }
